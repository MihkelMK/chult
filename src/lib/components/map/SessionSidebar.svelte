<script lang="ts">
  ...
  let activeSessionIndex = $derived(sessions.findIndex((session) => session.id === activeSessionId));
  let activeSessionGametime = $derived(
    sessions[activeSessionIndex]?.startGameTime ? globalGameTime - sessions[activeSessionIndex].startGameTime : 0
  );
</script>
