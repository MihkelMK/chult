<script lang="ts">
  ...
  let { session, path, isActive, isVisible, activeSessionDuration, activeSessionGametime, onToggleVisibility }: Props =
    $props();
...
<Collapsible.Root bind:open>
  <div class="rounded-lg border p-2">
    ...
    <div class="flex items-center justify-between gap-2">
      ...
      <!-- Visibility Toggle -->
      <Button variant="ghost" size="sm" onclick={() => onToggleVisibility(session.id)} class="h-8 w-8 p-0">
        {#if isVisible}
          <Eye class="h-4 w-4" />
        {:else}
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          ...
          <div>
            ...
            <span class="ml-1 font-medium">
              {formatGameTime(isActive ? activeSessionGametime : (session.endGameTime || 0) - session.startGameTime)}
            </span>
          </div>
          <div>
