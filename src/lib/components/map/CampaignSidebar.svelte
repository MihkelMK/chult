<script lang="ts">
  ...
  import type { DialogConfig, GameSessionResponse, MapMarker, TimeAuditLogResponse } from '$lib/types';
...
<SheetContent side="left" class="flex h-full w-80 flex-col p-0">
        ...
        <div>
          {#if hasActiveSession}
            <div class="space-y-4">
              ...
              {#if effectiveRole === 'dm'}
                <Button variant="destructive" size="sm" class="w-full cursor-pointer" onclick={handleEndSession}>
                  End Session
                </Button>
              {/if}
            </div>
          {:else if effectiveRole === 'dm'}
            <Button variant="default" size="sm" class="w-full cursor-pointer" onclick={handleStartSession}>
              Start Session
            </Button>
          {/if}
        </div>
  ...
  <div class="shrink-0 space-y-2 border-t p-6">
    {#if effectiveRole === 'dm'}
      <a href="/{campaignSlug}/settings" class={buttonVariants({ size: 'sm', variant: 'secondary', class: 'w-full' })}>
        Settings
      </a>
    {/if}
