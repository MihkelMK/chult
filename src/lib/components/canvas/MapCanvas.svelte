<script lang="ts">
  ...
  const calculatePanBounds = (elementSize: number, canvasSize: number, padding: number, scale: number) => {
  ...
  let { max: maxXPos, min: minXPos } = $derived(calculatePanBounds(scaledImageWidth, canvasWidth, dragBoundPaddingPX, zoom));

  let { max: maxYPos, min: minYPos } = $derived(calculatePanBounds(scaledImageHeight, canvasHeight, dragBoundPaddingPX, zoom));
...
{#snippet adjacentTileHighlight(hex: Hex)}
  <Group staticConfig={true} perfectDrawEnabled={false} shadowForStrokeEnabled={false} onclick={() => handleHexTrigger(hex.id)}>
    <RegularPolygon x={hex.centerX} y={hex.centerY} radius={hexRadius} sides={6} rotation={90} fill="#e2ffd3" opacity={0.5} />
    <RegularPolygon x={hex.centerX} y={hex.centerY} radius={hexRadius} sides={6} rotation={90} strokeWidth={2} stroke="black" />
    {@render tileCoords(hex)}
  </Group>
{/snippet}
...
<Stage
  ...
  <Layer staticConfig={true} listening={false}>
    <Image
      ...
      shadowOpacity={0.1}></Image>
  </Layer>

  <!-- Layer 2: Tiles -->
  <Layer x={xOffset} y={yOffset} clipX={gridClipX} clipY={gridClipY} clipHeight={gridHeight} clipWidth={gridWidth}>
    <!-- 1: Fog-of-war -->
  ...
  <Layer x={xOffset} y={yOffset} clipX={gridClipX} clipY={gridClipY} clipHeight={gridHeight} clipWidth={gridWidth}>
    ...
    <Group visible={(activeTool === 'select' || activeTool === 'paint') && selectedTiles.length > 0} listening={false}>
  ...
  <Layer x={xOffset} y={yOffset} clipX={gridClipX} clipY={gridClipY} clipHeight={gridHeight} clipWidth={gridWidth}>
    ...
    <MovementPaths {sessions} {pathsMap} visibleSessionIds={visiblePathSessions} {showPaths} {hexRadius} {hexGrid} />
    ...
    <Group listening={activeTool === 'interact'} oncontextmenu={handleBackgroundRightClick}>
      ...
      {#each markerTiles as { dmMarker, playerMarker, tile } (`${tile.col}-${tile.row}`)}
        ...
        {#if hasBoth}
          ...
          <MapMarker marker={dmMarker} tile={leftTile} radius={markerRadius} {onMarkerHover} {onMarkerClick} />
          <MapMarker marker={playerMarker} tile={rightTile} radius={markerRadius} {onMarkerHover} {onMarkerClick} />
        {:else if dmMarker}
          <!-- Single DM marker (centered) -->
          <MapMarker marker={dmMarker} {tile} radius={markerRadius} {onMarkerHover} {onMarkerClick} />
        {:else if playerMarker}
          <!-- Single player marker (centered) -->
          <MapMarker marker={playerMarker} {tile} radius={markerRadius} {onMarkerHover} {onMarkerClick} />
        {/if}
      {/each}
    </Group>
  </Layer>
