...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  ...
  <Dialog.Content class="sm:max-w-[425px]">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Manually adjust the global game time. This is useful for downtime, travel, or other events not tracked by movement.
      </Dialog.Description>
    </Dialog.Header>
    ...
    <div class="space-y-4 py-4">
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-2">
          <Input id="time-delta" type="number" step="0.5" bind:value={timeDelta} {disabled} placeholder="0" class="font-mono" />
          <Tooltip.Root>
            <Tooltip.Trigger>
        ...
        <p class="text-xs text-muted-foreground">Enter positive (forward) or negative (backward) time in days</p>
      </div>

      <div class="space-y-2">
        <Label for="time-notes" class="text-sm font-medium">Notes (optional)</Label>
        <Input id="time-notes" type="text" bind:value={notes} {disabled} placeholder="e.g., Long rest, travel montage..." />
      </div>

      {#if showConfirm}
        <div class="flex items-start gap-2 rounded-lg border bg-yellow-50 p-3 dark:bg-yellow-950/20">
          <CircleAlert class="mt-0.5 h-4 w-4 text-yellow-600 dark:text-yellow-500" />
          <div class="flex-1">
            <p class="text-sm font-medium text-yellow-800 dark:text-yellow-200">Large time adjustment</p>
            <p class="mt-1 text-xs text-yellow-700 dark:text-yellow-300">
              You're about to adjust time by {formattedDelta}. This will affect the global game time and cannot be easily
              undone.
            </p>
    ...
    <Dialog.Footer>
      {#if !showConfirm}
        <Button onclick={handleSubmit} disabled={disabled || timeDelta === 0} class="w-full">Adjust Time</Button>
      {/if}
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
