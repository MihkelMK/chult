<script lang="ts">
  ...
  const visibleSections = $derived(
    shortcutSections
      .map((section) => ({
        ...section,
        shortcuts: section.shortcuts.filter((shortcut) => !shortcut.onlyFor || shortcut.onlyFor === effectiveRole),
      }))
...
<Dialog.Root bind:open>
  ...
  <Dialog.Content class="max-h-[80vh] max-w-2xl overflow-y-auto">
    <Dialog.Header>
      ...
      <Dialog.Description>Quick reference for keyboard shortcuts available in the map view.</Dialog.Description>
    </Dialog.Header>
