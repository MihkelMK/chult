  ...
  <DialogContent class="sm:max-w-[600px]">
    ...
    <div class="grid gap-6 py-4">
            ...
            <button
              ...
              <div class="flex h-[50px] w-[50px] items-center justify-center">
                <Stage width={50} height={50}>
                  <Layer>
                    {#if type === 'settlement'}
                      <SettlementIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'dungeon'}
                      <DungeonIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'ruins'}
                      <RuinsIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'rest'}
                      <RestIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'landmark'}
                      <LandmarkIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'danger'}
                      <DangerIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'warning'}
                      <WarningIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {:else if type === 'generic'}
                      <GenericIcon x={25} y={25} radius={iconRadius} color={markerTypesMeta[type].color} />
                    {/if}
                  </Layer>
                </Stage>
              </div>
      ...
      <div class="grid gap-2">
        ...
        <Input id="title" type="text" placeholder="Enter marker name..." bind:value={title} required class="col-span-3" />
      </div>
      ...
      {#if isDM}
        <div class="flex items-center justify-between">
          <div class="space-y-0.5">
            ...
            <div class="text-sm text-muted-foreground">Players will see this marker on their map</div>
          </div>
