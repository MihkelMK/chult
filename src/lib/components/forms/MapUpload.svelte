<script lang="ts">
  ...
  let { mapExists, campaignSlug, onMapUploaded, onDeleted, mapType = 'dm', label }: Props = $props();
  ...
  {#if mapExists && !showUploadInterface}
    <div class="flex items-center justify-between rounded-lg">
      <div class="flex items-center gap-2">
        <div
          class="flex h-10 w-10 items-center justify-center {mapType === 'dm' ? 'bg-green-100' : 'bg-orange-100'} rounded-full">
      ...
      <div class="flex gap-2">
        <Button variant="outline" size="sm" onclick={() => (showUploadInterface = true)}>Replace</Button>
      ...
      <div>
        {#if mapExists}
          <div class="mb-4 flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onclick={() => (showUploadInterface = false)}>Cancel</Button>
          </div>
        {/if}
        ...
        <div
          ...
          {#if uploading}
            <div class="py-6">
              ...
              <p class="mt-1 text-xs text-muted-foreground">Large maps may take a moment to optimize</p>
            </div>
          {:else if previewUrl}
            <div class="space-y-3">
