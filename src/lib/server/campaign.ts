...
export async function getCampaignData(
  campaignId: number,
  isPlayerView = false
): Promise<((CampaignDataResponse | PlayerCampaignDataResponse) & { hasMapImage?: boolean }) | null> {
  ...
  const hasPlayerMapFilePromise = isPlayerView ? Promise.resolve(false) : hasMapImage(campaign.slug, 'player');
  ...
  const markersPromise = isPlayerView
    ...
    : db.select().from(mapMarkers).where(eq(mapMarkers.campaignId, campaignId)).orderBy(mapMarkers.createdAt);
  ...
  const [mapUrls, hasPlayerMapFile, revealed, markers, gameSessions, explorationPaths, auditLog] = await Promise.all([
    mapUrlsPromise,
...
export async function getCampaignStats(campaignId: number): Promise<CampaignStatsResponse> {
  const [stats] = await db
    .select({
      ...
      poisCount: sql<number>`count(case when ${mapMarkers.type} = 'poi' then 1 end)`.mapWith(Number),
      notesCount: sql<number>`count(case when ${mapMarkers.type} = 'note' then 1 end)`.mapWith(Number),
    })
    .from(campaigns)
