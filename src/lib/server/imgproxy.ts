...
export function generateMapUrl(
  ...
  if (options.preset) {
    const sourceUrl = hash ? `local:///${campaignSlug}/${filename}?v=${hash}` : `local:///${campaignSlug}/${filename}`;
...
export function generateMapVariants(campaignSlug: string, hash?: string, mapType: UserRole = 'dm'): ImageVariants {
  return {
...
export async function getMapUrls(slug: string, mapType: UserRole = 'dm'): Promise<MapUrlsResponse | undefined> {
