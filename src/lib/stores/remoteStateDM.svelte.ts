...
export class RemoteStateDM {
  ...
  constructor(campaignSlug: string, localState?: LocalStateDM) {
    ...
    if (localState) {
      localState.addEventListener('tile:revealed', (tile: TileCoords | RevealedTile) => {
        ...
        this.pending = this.pending.filter((op) => !(op.coords.x === tile.x && op.coords.y === tile.y && op.type === 'reveal'));
      });
      ...
      localState.addEventListener('tile:hidden', (tile: Pick<TileCoords, 'x' | 'y'>) => {
        ...
        this.pending = this.pending.filter((op) => !(op.coords.x === tile.x && op.coords.y === tile.y));
      });
    }
  }
  ...
  private isTileRevealed(coords: TileCoords): boolean {
    ...
    const isInSets = this.localState.revealedTilesSet.has(key) || this.localState.alwaysRevealedTilesSet.has(key);

    // Check pending operations
    const pendingReveal = this.pending.find(
      (op) => op.coords.x === coords.x && op.coords.y === coords.y && op.type === 'reveal'
    );
    const pendingHide = this.pending.find((op) => op.coords.x === coords.x && op.coords.y === coords.y && op.type === 'hide');
  ...
  private async processBatch() {
    ...
    const togglesToAlways = this.pending.filter((op) => op.type === 'toggleAlwaysRevealed' && op.alwaysRevealed);
    const togglesFromAlways = this.pending.filter((op) => op.type === 'toggleAlwaysRevealed' && !op.alwaysRevealed);
  ...
  revealTile(coords: TileCoords) {
    ...
    this.pending = this.pending.filter((op) => !(op.coords.x === coords.x && op.coords.y === coords.y));

    // Add reveal operation
    this.pending.push({ type: 'reveal', coords, timestamp: Date.now() });

    // Clear errors for this tile
    this.errors = this.errors.filter((err) => !(err.coords.x === coords.x && err.coords.y === coords.y));
  ...
  revealTiles(tiles: TileCoords[], alwaysRevealed: boolean = false) {
    ...
    this.pending = this.pending.filter((op) => !tilesToRevealSet.has(`${op.coords.x}-${op.coords.y}`));
  ...
  hideTiles(tiles: TileCoords[]) {
    ...
    this.pending = this.pending.filter((op) => !tilesToHideSet.has(`${op.coords.x}-${op.coords.y}`));
  ...
  toggleAlwaysRevealed(tiles: TileCoords[], alwaysRevealed: boolean) {
    ...
    this.pending = this.pending.filter((op) => !tilesToToggleSet.has(`${op.coords.x}-${op.coords.y}`));
  ...
  async adjustGlobalTime(delta: number, notes: string = '') {
    ...
    try {
      ...
      const exists = this.localState.timeAuditLog.some((entry) => entry.id === result.auditEntry.id);
      if (!exists) {
  ...
  async createMarker(data: {
    ...
    try {
      ...
      // Rollback on failure
      if ('mapMarkers' in this.localState.campaign) {
        this.localState.campaign.mapMarkers = this.localState.campaign.mapMarkers.filter((m) => m.id !== tempId);
        this.localState.markersById.delete(tempId);
        this.localState.markersVersion++;
      }
  ...
  async updateMarker(id: number, data: { title?: string; content?: string | null; visibleToPlayers?: boolean }) {
    if (!this.localState) {
      throw new Error('Local state not available');
    }
