<script lang="ts">
	...
	import DMViewAsPlayerBanner from '$lib/components/DMViewAsPlayerBanner.svelte';
...
<div class="min-h-screen bg-gradient-to-br to-blue-50 from-slate-50">
	{#if data.session.role === 'dm'}
		<DMViewAsPlayerBanner session={data.session} />
	{/if}
	<!-- Header -->
				...
				<div class="flex items-center space-x-6">
					<div class="hidden items-center space-x-4 text-sm text-gray-600 sm:flex">
						...
						<div class="flex items-center space-x-2">
							...
							<span
								>Points: <strong>{data.mapMarkers.filter((m) => m.type === 'poi').length}</strong
								></span
							>
						</div>
						<div class="flex items-center space-x-2">
