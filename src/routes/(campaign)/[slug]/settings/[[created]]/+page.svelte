<script lang="ts">
  ...
  let hasUnsavedPartyPosition = $derived(
    partyTokenX !== (data.campaign?.partyTokenX ?? null) || partyTokenY !== (data.campaign?.partyTokenY ?? null)
  );

  // Compute which map URLs to show based on viewAs toggle
  let displayMapUrls = $derived(
    viewAs !== 'dm' && hasPlayerMap && data.playerMapUrls ? data.playerMapUrls : data.dmMapUrls || data.mapUrls
  );
  ...
  async function savePartyTokenPosition() {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to save party token position';
      toast.error(errorMessage);
    } finally {
...
<Tooltip.Provider>
  <div class="container mx-auto max-w-4xl space-y-8 p-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center rounded-lg border bg-background/95 p-1 shadow-xs backdrop-blur-sm">
      ...
      <div class="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Share these links with your players and use the DM link yourself</CardDescription>
          </CardHeader>
          <CardContent class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="flex gap-2">
                ...
                <Button variant="outline" size="sm" onclick={() => copyToken(data.dmToken || '', 'dm')}>
                  {#if dmTokenCopied}
            ...
            <div class="space-y-2">
              ...
              <div class="flex gap-2">
                ...
                <Button variant="outline" size="sm" onclick={() => (showPlayerToken = !showPlayerToken)}>
                  {#if showPlayerToken}
                    <EyeOff class="h-4 w-4" />
                  {:else}
                    <Eye class="h-4 w-4" />
                ...
                <Button variant="outline" size="sm" onclick={() => copyToken(data.playerToken || '', 'player')}>
          ...
          <CardContent class="space-y-6">
            ...
            <div class="space-y-4">
              <div>
                ...
                <Slider id="offsetXSlider" type="single" bind:value={offsetX} min={-200} max={200} step={1} class="w-full" />
              </div>
              ...
              <div>
                ...
                <Slider id="offsetYSlider" type="single" bind:value={offsetY} min={-200} max={200} step={1} class="w-full" />
              </div>
            </div>

            <!-- Save Controls -->
            <Button onclick={saveHexGridConfig} disabled={!hasUnsavedChanges || saving} class="w-full">
              {#if saving}
                Saving...
          ...
          <CardContent class="space-y-4">
            {#if hasAnySessions}
              ...
              <div class="flex items-center gap-2 rounded-md bg-blue-50 p-3 text-sm text-blue-800">
                <span>
                  Party position cannot be changed manually once sessions have been created. Use the exploration system to move
                  the party.
                </span>
              </div>
            {:else}
          ...
          <CardContent class="space-y-6">
            ...
            {#if hasPlayerMap}
              <MapUpload
                ...
                label="Player Map" />
            {:else}
              <Button class="w-full cursor-pointer" size="sm" variant="ghost" onclick={() => (hasPlayerMap = true)}>
